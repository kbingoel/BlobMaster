...
def benchmark_optimal_config(
    ...
    # Calculate metrics
    rounds_per_minute = (num_games / elapsed) * 60
    seconds_per_round = elapsed / num_games
    examples_per_minute = (len(examples) / elapsed) * 60
    examples_per_round = len(examples) / num_games

    # Training time estimates (Phase 1: Independent Rounds)
    total_rounds = 500 * 10_000  # 500 iterations × 10,000 rounds
    training_days = (total_rounds / rounds_per_minute) / (60 * 24)
    ...
    print(f"Rounds/minute: {rounds_per_minute:.2f}")
    print(f"Seconds/round: {seconds_per_round:.3f}")
    print(f"Training examples: {len(examples):,}")
    print(f"Examples/round: {examples_per_round:.1f}")
    ...
    print(f"\n{'='*80}")
    print("TRAINING TIME ESTIMATE (Phase 1: Independent Rounds)")
    print("="*80)
    print(f"Full training (500 iterations × 10,000 rounds):")
    print(f"  Total rounds: {total_rounds:,}")
    print(f"  Estimated time: {training_days:.1f} days")
    print(f"  At rate: {rounds_per_minute:.1f} rounds/min")
    print("="*80)
...
def benchmark_mcts_variants(num_games: int = 200, device: str = "cuda"):
    ...
    for name, num_det, sims_per_det, total_sims in configs:
        ...
        rounds_per_min = (num_games / elapsed) * 60
        training_days = (500 * 10_000 / rounds_per_min) / (60 * 24)
        ...
        result = {
            ...
            "rounds": num_games,
            "elapsed": elapsed,
            "rounds_per_minute": rounds_per_min,
        ...
        print(f"\nResults:")
        print(f"  Rounds/minute: {rounds_per_min:.1f}")
    ...
    # Print comparison
    print("\n" + "="*80)
    print("MCTS CONFIGURATION COMPARISON (Phase 1: Independent Rounds)")
    print("="*80)
    print(f"{'Config':<12} {'Sims/Move':<12} {'Rounds/Min':<15} {'Training (days)':<18} {'Speedup'}")
    print("-"*80)

    baseline = results[0]['rounds_per_minute']
    for r in results:
        speedup = baseline / r['rounds_per_minute']
        print(f"{r['mcts_config']:<12} {r['total_sims']:<12} "
              f"{r['rounds_per_minute']:<15.1f} {r['training_days']:<18.1f} {speedup:.2f}x")
...
def main():
    ...
    with open(csv_path, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=[
            'timestamp', 'num_rounds', 'rounds_per_minute', 'seconds_per_round',
            'total_examples', 'examples_per_round', 'training_estimate_days'
        ])
        writer.writeheader()
        writer.writerow({
            'timestamp': results['timestamp'],
            'num_rounds': results['num_rounds'],
            'rounds_per_minute': results['rounds_per_minute'],
            'seconds_per_round': results['seconds_per_round'],
            'total_examples': results['total_examples'],
            'examples_per_round': results['examples_per_round'],
