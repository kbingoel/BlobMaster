#!/usr/bin/env bash
# Launch the 2026-05-06 long run. Wraps `scripts/run-train.sh train --config ...`
...
# runs while `iter < total_iterations` (174 here, so iter 173 is the
...
#   scripts/run-2026-05-06.sh --resume
set -euo pipefail
cd "$(dirname "$0")/.."

CONFIG="blob-train/run-2026-05-06.toml"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
LOG_BASE="$LOG_DIR/run-2026-05-06"
...
echo "  STOP    → checkpoints/run-2026-05-06/STOP (touch to ask for graceful exit)"
