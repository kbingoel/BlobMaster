...
**Status (2026-04-28, overnight battery): all three deferred levers ruled out.** Re-quantized iter_000014 against the existing 500-state BCAL with each of S8S8 activations, Entropy calibration, and per-block exclusion (8 separate runs, one per transformer block); validated all 10 against the same 0.95 / 1.00 static gate. Spread across all variants is **< 1pp on bid-argmax (0.842–0.852) and < 1.2pp on value-sign (0.932–0.944)** — none clears the gate; per-block exclusion at any depth moves the needle within calibration noise. The heads are already FP32 in every variant, so the residual error has to live in the trunk; 8 layers of cumulative quantization error at d_model=128 is the root cause exactly as flagged above. **Decision: INT8 is ruled out for this architecture, not parked.** The deferred-levers list is exhausted; the only remaining paths would be quantization-aware training (out of scope) or a wider model (d_model ≥ 256 — out of 7.5 scope). Full per-lever table in [self-play-profile.md](self-play-profile.md) "Follow-ups (2026-04-28)" and [logs/overnight-2026-04-27/SUMMARY.md](logs/overnight-2026-04-27/SUMMARY.md).
...
- **Muon optimizer for hidden 2D weights** *(implemented 2026-04-27, pulled ahead of trigger after Stage-2 throughput plateau)* — `blob-nn::muon::Muon` runs alongside AdamW. AdamW handles embeddings, biases, LN scales, and the three heads; Muon handles the transformer's 32 matrix weights (8 layers × {qkv, out, fc1, fc2}.weight). Mechanism: those weights are registered into a third param group `MUON_GROUP = 2`, with AdamW LR pinned to 0 for that group (so AdamW's step is a no-op — both moment-driven and decoupled-weight-decay terms scale by `lr`). Muon applies one heavy-ball momentum step (β=0.95) followed by 5 Newton-Schulz iterations with the Keller Jordan quintic coefficients `(3.4445, -4.7750, 2.0315)`, then an aspect-ratio rescale `sqrt(max(1, out/in))` so the per-element step magnitude tracks AdamW at the same LR. NS deliberately does *not* converge to true orthogonality at 5 iterations — the iteration leaves `S' ~ U(0.5, 1.5)`, which is enough for Muon's "balance the singular values" purpose. Wired in `train_step_with_grad_norms` between `clip_grad_norm` and `optimizer.step`; LR shares the AdamW schedule via `MUON_LR_SCALE = 1.0`.

  *Validation:* Stage-2 was abandoned because per-iter throughput was within run-to-run noise; Muon is the next-best ROI lever for total-run wall-clock (cuts iterations-to-target, not per-iter time). Validate alongside the 7.5 10-iter trajectory check before committing to 100 iters; if `eval_win_rate_lower95` at iter 10 falls below the 7.3c anchor, revert by setting `[training] enable_muon = false` (the runtime knob added 2026-04-28 — see status note below). The `[training] enable_muon` toggle in [TrainingLoopConfig](blob-nn/src/training_loop.rs) skips `muon.step` and overrides the `MUON_GROUP` AdamW LR back to the schedule LR so AdamW resumes updating the 32 matrices. *Do not* simply set `MUON_LR_SCALE = 0.0` — that would freeze the transformer matrices entirely (AdamW's lr is already 0 for them when `enable_muon = true`).

  *Caveat retained:* published Muon gains are mostly on >100M-param models; at 1.63M with d_model=128 the win may be smaller or wash into noise. Decision criterion remains the 7.5 trajectory, not theoretical priors.

  **Status (2026-04-28, overnight paired validation): no measurable benefit at this scale; defaulted off for 7.5.** Two paired 10-iter runs at the new fixed-5P7C / T=32 / target_batch=5 / 118 games-per-iter optimum, identical config except for `enable_muon`. In-loop eval triggered at iter 5; iter-9 row captured by direct `blobmaster-train evaluate iter_000009 vs iter_000000` after the runs (the `total_iterations = 10` config processes iters 0..9 and never reaches the iter-10 in-loop eval — see the off-by-one note in [AGENTS.md](AGENTS.md)). Results vs each run's own iter-0 anchor:

  | | iter-5 wr | iter-9 wr | iter-9 CI | iter-9 bid-success (current vs opp) |
  |---|---:|---:|---|---:|
  | A — Muon-on  | 0.604 | 0.609 | [0.539, 0.676] | 0.358 vs 0.315 |
  | B — Muon-off | 0.542 | 0.609 | [0.539, 0.676] | 0.380 vs 0.339 |
  | Δ (A − B)    | +0.062 (CIs overlap) | **0.000 (identical: 117/192 wins both)** | identical | −0.022 |

  The +6pp iter-5 gap was already inside Wilson noise (CIs overlapped); by iter 9 both runs have fully converged on win rate, score differential (+6.99 vs +6.96), *and* the 95% CI bounds. The only persistent signal is **bid-success +2.2pp in favour of Muon-off**, consistent across both checkpoints — Muon's update may be slightly worse for the bid head specifically (which is excluded from `MUON_GROUP`, so this is an indirect interaction via the trunk). The dev-plan §7.4d hold-back trigger ("> 5pp drop at iter 10") does **not** fire, so Muon may technically be retained. But the validation criterion was "Muon helps", and the answer at 1.63M / d_model=128 / ≤10 iters is empirically "no". **Decision: 7.5 ships with `enable_muon = false`.** The plumbing stays in tree as a parked escape hatch for any future architecture stretch (the published-Muon-gains caveat above predicts a non-trivial win starts to appear at ≥100M params). Full numbers + the same-machine paired iter-9 evals in [self-play-profile.md](self-play-profile.md) "Follow-ups (2026-04-28)" and [logs/overnight-2026-04-27/SUMMARY.md](logs/overnight-2026-04-27/SUMMARY.md).
...
**Status (2026-04-28): all 7.4-class levers settled by overnight battery — ready to launch.** The original "~3 min/iter ⇒ ~7 h for 100 iters" wall-clock estimate assumed INT8 + Muon would land; both ruled out empirically (see §7.4b 2026-04-28 status, §7.4d 2026-04-28 status). Real per-iter wall-clock is the post-7.4c-Stage-1 number: **~9 min/iter** on the FP32 / `target_batch = num_determinizations = 5` / T=32 stack. Updated wall-clock estimate: **~15 h for 100 iters + 20 evals**. Awkward but contiguous — resume-checkpoint at iter 50 gives one natural breakpoint; iter-25 / iter-75 also retained for fine-tuning candidates per the bullet above. Critical config for the 7.5 TOML: `[training] enable_muon = false`, `[self_play] use_int8 = false` *and* `fixed_player_count` omitted (or explicitly removed), `[mcts] num_determinizations = 5` / `target_batch = 5` / `sims_per_determinization = 100`, `[training] total_iterations = 101` (the iter-100 row only triggers when the loop reaches `iter == 100` per the `iter % eval_interval == 0` rule — see [AGENTS.md](AGENTS.md) off-by-one note). Mixed-player pipeline smoke-tested clean across n∈{4,5,6,7} in the same overnight battery (Block E, 5 iters, no panics).

**Expected wall-clock:** ~9 min/iter × 100 iters + ~20 evals ≈ **~15 h** end-to-end, resume-checkpoint at iter 50 (mid-run) and at every iter-25 boundary for fine-tuning candidates.
